    ...
    (function(global, factory) {
      ...
      function _query(selector, context = document, queryFn) {
        ...
        if (!parsed2.isContextSelector) {
          return parsed2.selector ? _doQuery(context, queryFn, parsed2.selector) : selector;
        }
        selector = "";
      ...
      function listenForBackgroundClose$1(modal2) {
        return on(document, pointerDown$1, ({ target }) => {
          if (last(active$1) !== modal2 || modal2.overlay && !modal2.$el.contains(target) || !modal2.panel || modal2.panel.contains(target)) {
            return;
          }
          once(
            document,
      ...
      function registerEvent(instance, { name, el, handler, capture, passive, delegate: delegate2, filter: filter2, self: self2 }) {
        ...
        instance._disconnect.push(
          on(
            ...
            name,
            delegate2 == null ? void 0 : delegate2.call(instance, instance),
      ...
      function callDisconnected(instance) {
        ...
        instance._disconnect.forEach((off2) => off2());
          ...
          },
          {
            ...
            async handler(_, item) {
              ...
              if (type === "image" || isImage(src)) {
                ...
                wrapInPicture(img2, item.sources);
                attr(img2, {
                  src,
                  ...pick(item, ["alt", "srcset", "sizes"]),
                  ...attrs
                });
                on(img2, "load", () => this.setItem(item, parent(img2) || img2));
      ...
      function ensureThumb(toggles, items) {
        for (const [i, toggle2] of Object.entries(toggles)) {
          if (items[i].thumb) {
            continue;
          }
          const parent2 = parents(toggle2).reverse().concat(toggle2).find(
            (parent22) => this.$el.contains(parent22) && (parent22 === toggle2 || $$(this.toggle, parent22).length === 1)
          );
          if (!parent2) {
            continue;
          }
          const media = $("img,video", parent2);
          if (media) {
      ...
      function parseSVG(svg2, icon) {
        ...
        return toNodes(fragment(svg2)).filter(isElement)[0];
      }
        ...
        update: [
          {
            read({ height: height$1, width: width2, margin, sticky: sticky2 }, types) {
              ...
              let [offset$1, offsetEnd] = [this.offset, this.offsetEnd].map(
                (value) => toPx(value, "height", sticky2 ? this.$el : referenceElement)
              );
