...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty='kl'):  
    ...
    reward_mask = create_reward_based_mask(data, response_length)  
    ...
    current_kl = safe_masked_mean(kld, mask=response_mask * reward_mask, axis=-1)  
...
def create_reward_based_mask(data: DataProto, response_length: int):  
    ...
    reward_condition = (token_level_scores < 0.1).all(dim=1).float()  # shape: (B,)
...
def safe_masked_mean(x: torch.Tensor, mask: torch.Tensor, axis=-1, eps=1e-8):
    ...
    safe_mean = torch.where(denom > 0, num / (denom + eps), torch.zeros_like(num))
    return safe_mean
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        if not self.config.do_search:
            ...
            for batch_dict in self.val_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    reward_tensor_lst.append(reward_tensor)
                    data_source_lst.extend(test_batch.non_tensor_batch.get('data_source', ['unknown'] * reward_tensor.shape[0]))

                    reward_extra = self.val_reward_fn.extra_metrics
                    search_count_lst.extend(reward_extra["search_count"])
                    search_presence_lst.extend(reward_extra["search_present"])
                    info_contains_answer_lst.extend(reward_extra["info_contains_answer"])
        ...
        search_counts = np.array(search_count_lst)
        ...
        data_source_info_answer = {}

        for i, ds in enumerate(data_sources):
            data_source_reward.setdefault(ds, []).append(reward_tensor[i].item())
            data_source_search_count.setdefault(ds, []).append(search_counts[i])
        ...
        for ds in data_source_reward:
            ...
            metric_dict[f'val/search_count/{ds}'] = np.mean(data_source_search_count[ds])
            metric_dict[f'val/search_present_ratio/{ds}'] = np.mean(data_source_search_presence[ds])
            metric_dict[f'val/info_contains_answer_ratio/{ds}'] = np.mean(data_source_info_answer[ds])
